...
**압구정은 같은 단지·같은 평형이라도 동에 따라 대지지분이 다릅니다.** 여러 차례 나눠 준공됐고
단지가 여러 필지에 걸쳐 있어 동별 대지권 산정 기준이 달랐기 때문입니다.
따라서 조사 단위는 **단지 × 동 × 평형**이며, 매물마다 등기부등본으로 개별 확인해야 합니다.

데이터는 두 군데에 넣을 수 있습니다.

- `landSharePyeongByDong` — `{"동명": {"전용면적": 대지지분평}}`. 정확한 값입니다.
- `landSharePyeong` — `{"전용면적": 대지지분평}`. 동 구분이 없을 때의 대체값입니다.

사이트는 동별 값을 먼저 찾고, 없으면 단지 공통값을 쓰며, 둘 다 없으면 '미입력'으로 둡니다.
동별 값이 여러 개면 시세표의 '지분당 단가'를 **범위**로 표시합니다.

```bash
npm run todo                              # 구역별로 몇 개 남았는지 + 우선순위 표시
node scripts/land-share-todo.mjs --csv    # 엑셀용 입력표 생성 (대지지분-입력표.csv)
```

CSV의 맨 오른쪽 칸만 채우면 됩니다. 우선순위는 **실거래 건수가 많은 조합** 순으로 잡습니다.

목록에 나오는 동은 **실거래에 동명이 공개된 건**에서만 도출됩니다. 국토교통부가 동을 비공개
처리한 거래는 `(동 미상)` 한 줄로만 나오고, 거래가 없던 동은 아예 나타나지 않습니다.
**실제 조사 대상은 이 목록보다 많습니다.**
